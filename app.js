(() => {
  ...
  function checklistText() { const lines = ["部署中心 - 运营管理系统部署交付清单", `文件路径：${state.credentials.path}`, `生成时间：${formatDate(new Date())}`, "", "注意：本文件包含敏感凭据，请妥善保管。", ""]; state.credentials.groups.forEach((group) => { lines.push(`【${group.title}】`); group.fields.forEach(([label, fieldValue]) => lines.push(`${label}：${fieldValue}`)); lines.push(""); }); return lines.join("\r\n"); }
